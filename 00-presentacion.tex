...
\begin{itemize}
    ...
    \begin{figure}
        \includegraphics[height=0.50\textheight]{img/conf_pedco.pdf}
        \captionsetup{textfont=tiny,labelformat=empty}
...
\begin{frame}
    ...
    \frametitle{Lista de correos}
    \framesubtitle{\texttt{cursada-ic-list@fi.uncoma.edu.ar}}

    Al finalizar la inscripción armamos una lista de correo con todas las
    personas que están cursando la materia.

\begin{itemize}
    \item Enviamos por mail las principales novedades (a todos los correos de
        la lista).
    \item Si no les llegan los correos avisar (mandar un mail para que le
        agreguemos).
    \item ¡Hacerse el habitó de revisar el correo de la facultad!
\end{itemize}
...
\begin{frame}
    ...
    \framesubtitle{\url{https://wguarani.uncoma.edu.ar/}}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Módulo I}: Martes \textbf{12:00} a \textbf{14:00} aula
            \textbf{13 (pagoda)}.
        \item \textbf{Módulo II}: Martes \textbf{14:00} a \textbf{16:00} aulas
            \textbf{i4}, \textbf{i5}.
    \end{itemize}
...
\begin{itemize}
    ...
    \item Se evalúan los contenidos prácticos y teóricos vistos en los
        trabajos prácticos.
    \item Si no se llega a aprobar un examen ni su recuperatorio, la persona
        estudiante se dice que \emph{queda libre}.
...
\begin{itemize}
    \item Puede acceder al examen de promoción. Se valúa la teoría vista hasta
        el momento (leer el \emph{apunte de la materia}).
...
\begin{itemize}
    \item Examen final regular: se evalúa la teoría (todo el \emph{apunte de
        la materia}).
...
\begin{frame}
    ...
    Si no se tiene la materia \emph{cursada} (ni se esta cursando):

\begin{itemize}
    \item Examen final libre, se evalúa en dos días diferentes:
        \begin{itemize}
            \item Parte práctica: se evalúan los temas de los trabajos
                prácticos.
            \item Parte teórica: solo se rinde si se aprobó la parte práctica.
                Se evalúan los temas vistos en teoría.
        \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item Los finales se aprueban con una nota mayor o igual a 4 \footnote{Eso
        no quiere decir que se tenga un $40\%$ de las respuestas correctas}
        (con un $60\%$ de las respuestas correctas).
