...
Representación de números racionales y reales}
\author{}
\date{}
...
\item Racionales:
    \begin{itemize}
        \item Punto Fijo (\emph{PF}).
        \item Punto Flotante (\emph{minifloat}).
    \end{itemize}
\end{itemize}
\end{frame}

\begin{frame}{Números racionales y fraccionarios}
\begin{itemize}
    \item Los números racionales son todos aquellos números que pueden ser
        representados por la fracción o razón, entre dos enteros.
...
\begin{itemize}
    \item Por otro lado, existen números dentro de la recta numérica real que
        no son racionales (no existe una fracción entre enteros que los
        represente) pero tienen una representación decimal con parte entera y
        una parte decimal \textbf{infinita} y \textbf{no periódica}.\pause
...
\begin{frame}{Números reales}
    \begin{figure}
    \centering
    \includegraphics[height=0.9\textheight]{img/reales.pdf}
...
\begin{frame}{Expresión General Extendida}
    ...
    La coma (o punto decimal) señala el lugar donde los exponentes de la base
    de la expresión general se hacen negativos.

    Sea un número \(N_{(b)} =
    X_{k}X_{k-1}...X_{1}X_{0},X_{-1}X_{-2}...X_{-q}\), donde cada \(X_{i}\)
    son sus dígitos en base \(b\). Su representación en decimal \(W_{(10)}\):
...
\begin{enumerate}
    ...
    \item La parte fraccionaria se multiplica por 2 y se toma la parte entera
        del resultado. Este dígito binario se agrega al resultado final.

    \item Se repite el paso \(3\) con la nueva parte fraccionaria obtenida,
        hasta que ésta sea 0, o hasta lograr la precisión deseada.

    \item La parte entera de cada uno de los resultados del paso \(4\), forman
        los dígitos del número en la base en orden de izquierda a derecha.
...
\item La parte entera se convierte a base 2 como entero sin signo (utilizando
    el método de la división) dando \(11_{(2)}\).\pause

\item Expresamos la parte fraccionaria:

    \begin{enumerate}

    \item Multiplicamos la parte fraccionaria hasta obtener cero o una
...
\begin{frame}{Representación de racionales en \textbf{bits}}

    ¿Cómo podemos representar los números racionales en la computadora?

    Veremos dos sistemas de representación:
\begin{itemize}
    \item Punto fijo.
    \item Punto flotante (\emph{minifloat}).
\end{itemize}
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \item Expresar la parte entera y fraccionaria en la cantidad adecuada
            de \textbf{bits}. Si es necesario completar con ceros, a la
            izquierda para la parte entera, y a la derecha en la fraccionaria.
        \item Si el número es negativo, aplicar la operación de complemento
            como si fuera un número representado en \emph{Complemento a 2}.
    \end{enumerate}
\end{itemize}
\end{frame}
...
\framesubtitle{Truncamiento}
    ...
    ¿Qué pasa si no es suficiente la cantidad de bits que tenemos para la
    parte fraccionaria?  ¿Es realmente \(1000\,1011 = -7,33_{(10)}\) con
    \emph{PF(8,4)}?
    \pause
...
\begin{itemize}
    \item El coeficiente \textit{m} debe ser un valor mayor o igual que 1 y
        menor que 10.
    \pause
\item Ejemplo: \(t = 0,\overline{3} \times 10^{-17}s
...
\begin{itemize}
    ...
    \item El formato \emph{IEEE 754}, definido por la \emph{IEEE}
        (\emph{Institute of Electrical and Electronics Engineers}), define
        versiones de precisión media (16\bit{}), simple (32\bit{}), doble
        (64\bit{}), y cuádruple (128\bit{}), y óctuple (256\bit{}).
\end{itemize}
...
\begin{enumerate}
    \item Separar el signo y escribir el valor absoluto de \textit{n} en base
        2.
...
\item Normalizado: \(1,011 \times 2^2\)  por lo que la mantisa es \(m=011\).
        \pause
    \item El exponente se representa en exceso a 7: \(e = 2 + 7 = 9 =
        1001_{2}\)
        \pause
    \item Resultado Final: \(1100\,1011\). En hexadecimal \textbf{0xCB}.
\end{itemize}
\end{frame}
...
A pesar del incremento en la precisión, aun puede causar problemas: ¿Cuánto
    es \(0.2 + 0.1\)? (ver en la consola con \texttt{python3}) \pause

    \begin{figure}
    \centering
    \includegraphics[width=0.5\textwidth]{img/floaterror.pdf}
    \captionsetup{labelformat=empty}
    \caption{}
    \end{figure}
...
\framesubtitle{Truncamiento}
    ...
    En \emph{punto flotante}, igual que con \emph{punto fijo}, se pueden
    producir errores de truncamiento.
    ...
    Al intentar representar \(0.1\) en \emph{minifloat} obtenemos como
    resultado \textbf{0x1C}. Pero al convertirlo de vuelta a decimal obtenemos
    como resultado \(0.09375\).

    Como \(0.1 - 0.09375 = 0.00625\), entonce el error de truncamiento es
    \(0.00625\).

\end{frame}

\begin{frame}{Números representables en \emph{Punto Fijo} y \emph{Punto
    flotante}}

    Comparemos los números representables entre \emph{minifloat} y
    \emph{PF(8,2)} ¿Tendrán el mismo rango? ¿Cuál sera más preciso?
    \pause

    \begin{figure}
    \centering
    \includegraphics[width=0.9\textwidth]{img/rectaMinifloat.pdf}
    \captionsetup{labelformat=empty}
    \caption{Números representables con \emph{minifloat}.}
    \end{figure}

    \begin{figure}
    \centering
    \includegraphics[width=0.9\textwidth]{img/rectaPuntoFijo.pdf}
    \captionsetup{labelformat=empty}
    \caption{Números representables con \emph{Punto Fijo (8,2)}.}
    \end{figure}
