...
\begin{equation*}
    ...
    \pause
    =
    X_{0}{\times}b^{0}+X_{1}{\times}b^{1}...
    X_{k-1}{\times}b^{k-1}+X_{k}{\times}b^{k}
    \pause
    = Q_{(10)}
\end{equation*}
\pause
...
\begin{itemize}
    \item
        $60_{(7)} = \pause
        0{\times}7^0+6{\times}7^1 = \pause
        42_{(10)}$\pause
    \item $\mathrm{2A}_{(16)} = \pause
        (10){\times}(16)^0+2{\times}(16)^1 = \pause
        42_{(10)}$\pause
    \item $1120_{(3)} = \pause
        +0{\times}3^0+2{\times}3^1+1{\times}3^2+1{\times}3^3 = \pause
        42_{(10)}$
\end{itemize}
...
\framesubtitle{Conversión de decimal a otras bases}
    ...
    $b$).
    \begin{enumerate}
    \item Realizar la división \textbf{entera} del número origina por la base
        destino, anotando cociente y resto.
    \item Mientras el cociente sea mayor a cero:
        ...
        \end{itemize}
    \item Finalmente escribimos \textbf{todos los restos en orden inverso a
        como aparecieron}. El numeral resultante es $Q_{(10)}$ representado en
        base $b$.
    \end{enumerate}
...
\begin{figure}
    \centering
    \includegraphics[height=0.75\textheight]{img/hexadump.pdf}
    \captionsetup{textfont=tiny,labelformat=empty}
    \caption{Volcado en hexadecimal de un binario ejecutable}
\end{figure}
